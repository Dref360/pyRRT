        ...
        if 'obstacles' in json_obj:
            ...
            if shared.dimensions == 2:
                ...
                for obs in obstacles:
                    this_obstacle = obstacles[obs]
                    x = this_obstacle["x"] * shared.x_range
                    y = this_obstacle["y"] * shared.x_range
                    z = this_obstacle["z"] * shared.x_range
                    width = this_obstacle["width"] * shared.x_range
                    height = this_obstacle["height"] * shared.x_range
                    depth = this_obstacle["depth"] * shared.x_range
                    shared.obstacles.append(obstacle.Obstacle([x, y, z], [width, height, depth]))
        if 'nodes' in json_obj:
            ...
            if shared.dimensions == 2:
                ...
                for my_node in nodes:
                    ...
                    if this_node["type"] == "root":
                        x = this_node["x"] * shared.x_range
                        y = this_node["y"] * shared.x_range
                        z = this_node["z"] * shared.x_range
                        for obs in shared.obstacles:
                            if obs.collides_with([x, y, z]):
                                print("Error: Specified root node collides with an obstacle.".format(infile),
                                      file=sys.stderr)
                                exit(1)
                        shared.nodes.append(node.Node([x, y, z], None, "root"))
                        root_set = True
                        ...
                        for obs in shared.obstacles:
                            if obs.collides_with([x, y, z]):
                                print("Error: Specified root node collides with an obstacle.".format(infile),
                                      file=sys.stderr)
                                exit(1)
                        shared.goal = node.Node([x, y, z], None, "goal")
                        goal_set = True
